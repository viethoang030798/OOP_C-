...
int main(){
  ...
  for(int i=0; i<n;i++)
  {
     ...
     if(40 <= s[i].point && s[i].point <= 100)
     {
     	a[i] = s[i].point - 40;
     	b[i] = a[i]/15;
       cout <<s[i].name << " PASS THE COURSE!"<<endl<< "Grade: "<< g[i] + b[i] +1 <<endl; //because when point is from 40 to 100, grade already is 1, so have to add 1
     }
